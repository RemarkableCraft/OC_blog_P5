<section id="formPost" class="form">
	<div class="container">
		...
		<form action=" <?php if ($lien === 'action=createPost'): ?> ?action=addPost <?php else: ?> ?action=updatePost&id=<?= $post['idPost'] ?> <?php endif ?>" method="POST" novalidate class="needs-validation php-email-form shadow" data-aos="fade-left">
			<!-- ===== Title POST ===== -->
